...
| `/temukan` | ◐ PPR | `async` di `<Suspense>`, `force-dynamic` dihapus |
| `/cv/[username]` | ◐ PPR | `async` di `<Suspense>` |
| `/beranda` | ◐ PPR | `DashboardContent` di `<Suspense>` + skeleton |
| `/notifikasi` | ◐ PPR | `NotificationsContent` di `<Suspense>` + skeleton |
| `/cv/edit` | ◐ PPR | `CVEditContent` di `<Suspense>` + skeleton |
| `/admin/review` | ◐ PPR | `force-dynamic` dihapus, `ReviewClient` di `<Suspense>` |
| `/pengenalan` | ◐ PPR | `OnboardingLoader` di `<Suspense>` |
...
- [x] Hapus `dynamic = "force-dynamic"` di `/temukan/page.tsx`
- [x] Hapus `dynamic = "force-dynamic"` di `/admin/review/page.tsx`
- [x] Restruktur `/temukan/page.tsx`: `async` dalam `<Suspense>`
- [x] Restruktur `/cv/[username]/page.tsx`: `async` dalam `<Suspense>`

### Fase 2 ✅ — Suspense Boundaries Baru
- [x] `/beranda/page.tsx`: `DashboardContent` dalam `<Suspense>` + skeleton
- [x] `/notifikasi/page.tsx`: `NotificationsContent` dalam `<Suspense>` + skeleton
...
- [x] `/pengenalan/page.tsx`: `OnboardingLoader` dalam `<Suspense>`
...
**Yang berubah:**
- `DEFAULT_LOGIN_REDIRECT` → `/beranda` (dari `/`)
- Proxy redirect logged-in users dari `/` ke `/beranda`
- Landing page menjadi **○ Static** murni (tanpa session/redirect logic)
- Auth guard (no session → /masuk) ditangani proxy, layout hanya handle username/onboarding
...
## Intercepting Routes — Modal Detail CV dari Halaman Temukan

**Intercepting Routes** memungkinkan navigasi dari `/temukan` ke `/cv/[username]` ditangkap dan ditampilkan sebagai **modal/overlay** di atas halaman `/temukan`, tanpa meninggalkan konteks pencarian. URL tetap berubah ke `/cv/[username]` (bisa di-share), dan jika di-refresh atau dibuka langsung, tampil halaman CV penuh.

### Cara Kerja

Ketika user klik kartu CV di `/temukan`:
1. Next.js menangkap navigasi ke `/cv/[username]` karena ada file `(..)cv/[username]/page.tsx` di dalam parallel route `@modal`
2. Konten CV dirender di dalam modal di atas halaman `/temukan`
3. URL berubah ke `/cv/[username]`
4. User klik close modal → navigasi balik ke `/temukan` (bukan ke halaman sebelumnya)
...
│       └── (..)cv/
│           └── [username]/
│               └── page.tsx  ← intercept `/cv/[username]` dari `/temukan`
...
### Prasyarat
- Parallel route `@modal` di `(dashboard)/temukan/`
- Layout `(dashboard)/layout.tsx` perlu menerima dan merender prop `modal`
- Komponen modal (sheet/dialog) yang membungkus konten CV
- Link di `temukan-client.tsx` mengarah ke `/cv/[username]` (sudah ada)

### Checklist Implementasi ✅
- [x] Buat `src/app/(dashboard)/temukan/@modal/` dengan `default.tsx` dan `[...catchAll]/page.tsx`
- [x] Buat `src/app/(dashboard)/temukan/@modal/(..)cv/[username]/page.tsx` — intercepting route
- [x] Buat komponen modal (`CvDetailModal`) dengan shadcn `Dialog` + `CandidateDetailClient`
- [x] Buat `src/app/(dashboard)/temukan/layout.tsx` untuk render slot `@modal`
